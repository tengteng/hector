...
func (c *CategoryFeatureCombination) OneCVAUC(dataset0 *core.RawDataSet,
	combines []core.CombinedFeature, total_cv, cv int) float64 {
...
func (c *CategoryFeatureCombination) FindCombination(
	dataset *core.RawDataSet) []core.CombinedFeature {
	features := []string{}
	...
	for i, fi := range features {
		c.feature_combinations = append(c.feature_combinations,
			core.CombinedFeature{fi})
		for j, fj := range features[i+1:] {
			candidate_column_combines = append(
				candidate_column_combines,
				core.CombinedFeature{fi, fj})
			for k, fk := range features[i+j+1:] {
				candidate_column_combines = append(
					candidate_column_combines,
					core.CombinedFeature{fi, fj, fk})
	...
	for {
		...
		for i, column_combines := range candidate_column_combines {
			...
			for cv := 0; cv < total_cv; cv++ {
				ave_auc += c.OneCVAUC(dataset, temp_combines,
					total_cv, cv)
			}
		...
		c.feature_combinations = append(c.feature_combinations,
			candidate_column_combines[best_combines])
