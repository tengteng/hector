...
func (self *StringBuilder) Int64(value int64) *StringBuilder {
	self.buffer.WriteString(strconv.FormatInt(value, 10))
	return self
}

func (self *StringBuilder) UInt64(value uint64) *StringBuilder {
	self.buffer.WriteString(strconv.FormatUint(value, 10))
	return self
}
...
func (self *StringBuilder) WriteToFile(filename string) error {
	return ioutil.WriteFile(filename, self.buffer.Bytes(), 0600)
}
